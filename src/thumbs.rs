...
pub fn generate_video(
    ...
    for seek in ["5", "0"] {
        let ok = std::process::Command::new("ffmpeg")
            .args([
                "-ss", seek, "-i", path, "-vframes", "1", "-vf", &vf, "-q:v", &qscale, "-y",
                &out_str,
            ])
...
pub fn generate_video_from_archive(
    ...
    let tmp_path = std::env::temp_dir().join(format!("media_idx_thumb_{hash}.{ext}"));
...
pub fn generate_3d(
    ...
    if stlthumb_supported && stl_thumb_available() {
        let tmp = std::env::temp_dir().join(format!("media_idx_3d_{hash}.{ext_lower}"));
        std::fs::write(&tmp, data)?;
        let result = generate_3d_stlthumb(tmp.to_string_lossy().as_ref(), hash, thumb_dir, size);
        ...
        match result {
            ...
            Err(e) => {
                ...
                eprintln!(
                    "  {} stl-thumb failed, using internal renderer: {e}",
                    "⚠".yellow()
                );
            }
        }
...
fn generate_3d_stlthumb(model_path: &str, hash: &str, thumb_dir: &Path, size: u32) -> Result<()> {
    let out = thumb_path(thumb_dir, hash);
    std::fs::create_dir_all(out.parent().unwrap())?;
...
fn render_isometric(verts: &[[f32; 3]], tris: &[[usize; 3]], size: u32) -> image::RgbImage {
    ...
    for tri in tris {
        ...
        if a >= proj3d.len() || b >= proj3d.len() || c >= proj3d.len() {
            continue;
        }
        let va = proj3d[a];
        let vb = proj3d[b];
    ...
    for y in 0..size {
        for x in 0..size {
            ...
            img.put_pixel(
                x,
                y,
                image::Rgb([(r / 4) as u8, (g / 4) as u8, (b / 4) as u8]),
            );
        }
    }
...
fn parse_stl_binary(data: &[u8]) -> (Vec<[f32; 3]>, Vec<[usize; 3]>) {
    ...
    let n =
        (u32::from_le_bytes([data[80], data[81], data[82], data[83]]) as usize).min(MAX_VERTS / 3);
...
fn parse_3mf(data: &[u8]) -> (Vec<[f32; 3]>, Vec<[usize; 3]>) {
    ...
    let model_idx = (0..archive.len()).find_map(|i| {
        ...
        if e.name().to_lowercase().ends_with(".model") {
            Some(i)
        } else {
            None
        }
    ...
    if archive
        .by_index(idx)
        .ok()
        .and_then(|mut e| e.read_to_string(&mut content).ok())
        .is_none()
    {
        return (vec![], vec![]);
    }
            ...
            Ok(Event::Empty(ref e)) | Ok(Event::Start(ref e)) => match e.name().as_ref() {
                b"vertex" => {
                    ...
                    for attr in e.attributes().flatten() {
                        let val: f32 = std::str::from_utf8(&attr.value)
                            .ok()
                ...
                b"triangle" => {
                    let mut v = [0usize; 3];
                    for attr in e.attributes().flatten() {
                        let i: usize = std::str::from_utf8(&attr.value)
                            .ok()
                            .and_then(|s| s.parse().ok())
                            .unwrap_or(0);
                        match attr.key.as_ref() {
